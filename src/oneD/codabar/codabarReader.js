zxing.oneD.codabar.reader = {
    ...
    decodeRow: function (bitRow, options) {
        ...
        var startOffset = this._findStartPattern(),
            ...
            chars = [],
            startEndEncodings = this.START_END_ENCODINGS,
            alphabet = this.ALPHABET,
            alphabetString = this.ALPHABET_STRING,
        ...
        do {
            ...
            if (chars.length > 1 && startEndEncodings.indexOf(alphabet[charOffset]) !== -1) {
                break;
            }
        ...
        if (startEndEncodings.indexOf(alphabet[chars[0]]) === -1 ||
            startEndEncodings.indexOf(alphabet[chars[chars.length - 1]]) === -1 ||
            // Remove stop/start characters character and check if a long enough string is contained.
            chars.length <= this.MIN_CHARACTER_LENGTH
        ) {
        ...
        for (i = 0; i < chars.length; i++) {
            chars[i] = alphabetString.charAt(chars[i]);
        }
    ...
    _validatePattern: function (start, chars) {
        var sizes = new Uint32Array([0, 0, 0, 0]),
            ...
            theCounters = this._counters,
            charEncoding = this.CHARACTER_ENCODINGS,
            maxAcceptable = this.MAX_ACCEPTABLE,
        ...
        for (i = 0; i < 2; i++) {
            ...
            maxs[i] = mins[i + 2];
            maxs[i + 2] = (sizes[i + 2] * maxAcceptable + padding) / counts[i + 2];
        }
        ...
        for (i = 0; true; i++) {
            ...
            for (j = 6; j >= 0; j--) {
                ...
                if (size < mins[category] || size > maxs[category]) {
                    return -1;
                }
                pattern = pattern >> 1;
            }
    ...
    _findStartPattern: function () {
        var startEndEncodings = this.START_END_ENCODINGS,
            alphabet = this.ALPHABET,
            i = 1,
            charOffset;
        ...
        for (; i < this._counterLength; i += 2) {
            ...
            if (charOffset !== -1 && startEndEncodings.indexOf(alphabet[charOffset]) !== -1) {
                var patternSize = 0;
                for (var j = i; j < i + 7; j++) {
    ...
    _toNarrowWidePattern: function (position) {
        ...
        for (i = 0; i < charEncoding.length; i++) {
            if (charEncoding[i] === pattern) {
                return i;
            }
        }
